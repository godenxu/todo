...
async function main() {
  ...
  section('图表页"按任务"：优先级/来源/标签合并到同一行，来源/标签改用横向列表（不再是纵向柱状图）');
  ok('三者的标题合并成了一个面板"优先级 / 来源 / 标签分布"', h.includes('优先级 / 来源 / 标签分布'));
  ok('不再是三个各自独立的面板标题', !h.includes('▤ 优先级分布') && !h.includes('▤ 来源分布') && !h.includes('▤ 标签分布'));
  ...
  section('图表页"按工作"：各年度工作数量改名，各职责工作数量改用横向列表');
  ok('标题改成了"各年度工作数量 / 各职责工作数量"', h.includes('各年度工作数量 / 各职责工作数量'));
