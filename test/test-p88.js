...
async function main() {
  ...
  section('⑮：彻底删除 vs 同时有人在编辑——把当前行为钉死（避免以后改坏了没人发现）');
  /* 现在的规则（applyPurged）：墓碑只剔掉"最后修改时间早于墓碑时间"的记录。
     所以如果有人在别人彻底删除之后又改过这条，它会重新出现。
     这是为职责/工作准备的——它们的主键是人填的编号，删掉之后完全可能再建一个同编号的新记录，
     一律按墓碑剔会误杀那条新的。任务/里程碑的主键是随机 id，不存在这种情况，
     所以对它们来说这条规则只会带来"被彻底删掉的东西又回来了"。
     没有顺手改掉，是因为改了会让"彻底删除"变得更具破坏性（把对方正在编辑的内容直接抹掉），
     而彻底删除本身是管理员权限、且有明确的不可撤销提示。这里先把行为测住，
     真要调整时能立刻看出影响面。 */
  file = mkFile(seed);
  ...
  devSync(b, file);
  ok('当前行为：墓碑之后还被编辑过的记录会重新出现（已知取舍，见上面注释）',
    !!findTask(file.payload, 'x1'), '如果这条挂了，说明有人改了 applyPurged 的口径，请确认是有意为之');
