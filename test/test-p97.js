...
async function main() {
  ...
  section('五、"本机数据整批换掉"的动作，必须把基线和写入链标记一起清干净');
  {
    ...
    /* P120 追加一处：首次连接到一个还没有数据文件的文件夹、用本机数据建新文件（seedFromLocal）——
       本机跟这份新文件毫无对账关系，同样是"记忆整批作废"的时刻 */
    ok('★六个"整批换掉数据/整批作废记忆"的入口都走这一个收口函数（备份恢复 / 以共享文件为准重置 / 播种演示数据 / 断开连接 / 首次连接 / 首次连接空文件夹用本机数据建新文件）',
      n === 6, n);
